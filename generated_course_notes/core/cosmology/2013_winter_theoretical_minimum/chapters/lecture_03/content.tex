...
\caption{A clean reconstruction of the lecture's two pictures: flat space as nested circles, and the sphere as circles that grow, reach a maximum, and then shrink back to zero.}
\end{figure}
...
\caption{Left: the shell-size factors \(r\), \(\sin r\), and \(\sinh r\), which control how many objects lie on a shell at distance \(r\). Right: the angular-size laws \(d/r\), \(d/\sin r\), and \(d/\sinh r\). On the sphere the apparent size turns around; in hyperbolic space it shrinks exponentially fast.}
\end{figure}
