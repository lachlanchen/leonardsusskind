...
\node[below right] at (2.2,-0.75) {$\mathcal{I}^-$};
\node[above right] at (2.0,0.95) {$\mathcal{I}^+$};
\end{tikzpicture}
...
Finally, the proper-time statement must be kept straight. From a generic horizon-crossing event to the singularity there is only a finite proper time. Only the ideal corner where the singularity meets \(\mathcal{I}^+\) is infinitely far away, and trying to cling to that corner would require infinite acceleration and infinite energy. It is not an escape route.
