...
\end{tikzpicture}
\caption{The constructive picture used in the lecture. A two-sphere is built from one-spheres whose radius grows and shrinks, and a three-sphere is obtained by repeating the same pattern one dimension higher.}
\end{figure}
...
\node at (0,-2.85) {Fixed-\(\alpha\) spheres centered on us grow, then shrink};
\end{tikzpicture}
\caption{Observer-centered spherical geometry drawn in two-dimensional cross-section. In the real three-dimensional case the fixed-\(\alpha\) surfaces are two-spheres around us: they first grow with distance and then, after a maximum, begin to shrink.}
\end{figure}
...
\node at (0,-2.75) {A Poincar\'e-disk style picture: equal objects shrink toward the boundary};
\end{tikzpicture}
