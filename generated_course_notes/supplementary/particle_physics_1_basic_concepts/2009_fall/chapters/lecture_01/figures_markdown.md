...
- `lecture_01_figure_04.png` preserves the derivational organization of the board. The older relation remains boxed at left, the intermediate conversion formulas are stacked above and center-right, and the final result `\omega = \frac{2\pi c}{\lambda}` is boxed below. That boxed lower-right placement matters for note writing because it shows which equation Susskind is treating as the takeaway.
...
- `lecture_01_figure_05.png` should remain visible. Pair it with a careful typeset explanation that in natural units `c=1`, the rest-energy relation is written as
  \[
  E = m.
  \]
...
- In `lecture_01_figure_05.png`, the lower relation beneath `E = m` is not fully legible. It appears to be a dimensional form involving mass, length squared, time squared, and energy, but that should be treated cautiously unless confirmed elsewhere in the transcript or surrounding frames.
